...
assert.match(await store.makePaymentQr(payload), /^data:image\/png;base64,/);
assert.equal(store.isFinalPaymentStatus('success'), true);
assert.equal(store.isFinalPaymentStatus('failed'), true);
assert.equal(store.isFinalPaymentStatus('pending'), false);
